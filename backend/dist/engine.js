...
export function joinRoom(room, name) {
    ...
    const targetValue = room.config.startingAssets.TKL * BASE_PRICES.TKL +
...
function drawEventAndUpdateMarket(room) {
    ...
    const applyDelta = (target, eff) => {
        ;
        ['TS', 'TG', 'TKL', 'C'].forEach((k) => {
            const d = eff?.[k];
            if (typeof d === 'number')
                target[k] = Math.max(1, target[k] + d);
        });
    };
...
export function startRound(room, index) {
    const now = Date.now();
...
function addLog(room, playerId, msg) {
    if (!room.currentRoundLogs[playerId])
        room.currentRoundLogs[playerId] = [];
    room.currentRoundLogs[playerId].push(msg);
}
export function lockAction(room, playerId, action, payload) {
    if (room.phase !== 'IN_ROUND' || !room.round)
        throw new Error('Not accepting actions');
    ...
    if (everyoneLocked || timePassed)
        revealRound(room);
}
function applyOutcomeForPair(room, a, b) {
    const rc = room.round;
    const ca = rc.choices[a.id];
    const cb = b ? rc.choices[b.id] : undefined;
    // Helper to buy Crypto using TS at current round prices
    const buyCrypto = (p, choice) => {
        const wantC = Math.floor(Number(choice?.payload?.amountC || 0));
        if (!Number.isFinite(wantC) || wantC <= 0)
            return;
...
function applyEndOfRoundFees(room) {
    // Gold custody fee
    const feeFor = (tkl) => (tkl >= 12 ? 2 : tkl >= 8 ? 1 : 0);
    let fee1Count = 0;
    let fee2Count = 0;
    let deductedTS = 0;
    const feeDetails = [];
    ...
    if (room.market.inflation) {
        Object.values(room.players).forEach((p) => {
            ...
            if (p.assets.TG >= 10)
                p.assets.TS = Math.max(0, p.assets.TS - 1);
        });
    }
...
function ensureMinimumTS(room, minTS = 1) {
    const order = ['TG', 'TKL', 'C'];
    Object.values(room.players).forEach((p) => {
...
function recalcNFromOutcomes(room) {
    const rc = room.round;
    let coop = 0;
    let betray = 0;
    ...
    if (coop > betray)
        room.market.N = clampN(room.market.N + 1);
    else if (betray > coop)
        room.market.N = clampN(room.market.N - 1);
}
export function revealRound(room) {
    ...
    for (const { a, b } of room.round.pairings) {
        ...
        if (pa)
            applyOutcomeForPair(room, pa, pb);
    }
    ...
    if (fees.fee1Count || fees.fee2Count) {
        const parts = [];
        if (fees.fee1Count)
            parts.push(`-1 TS: ${fees.fee1Count} người (8–11 TKL)`);
        if (fees.fee2Count)
            parts.push(`-2 TS: ${fees.fee2Count} người (≥12 TKL)`);
    ...
    if (room.round) {
        Object.keys(room.round.choices).forEach((pid) => {
            ...
            if (a)
                push(pid, `Bạn đã chọn hành động: ${a}`);
        });
    }
...
export function nextRoundOrEnd(room) {
    if (!room.round)
...
export function indicativePrices(room) {
...
function priceTSFor(room, asset, amount) {
...
export function createOffer(room, sellerId, asset, amount) {
    if (room.phase !== 'IN_ROUND' || !room.round)
        throw new Error('Chỉ giao dịch trong vòng đang diễn ra');
    if (Date.now() >= room.round.lockUntilTs)
        throw new Error('Hết thời gian đàm phán vòng này');
    ...
    if (asset === 'TS')
        throw new Error('Không bán TS trực tiếp (TS là đơn vị thanh toán)');
    amount = Math.floor(amount);
    if (seller.assets[asset] < amount)
        throw new Error('Không đủ tài sản để rao bán');
...
export function fillOffer(room, buyerId, offerId) {
    ...
    if (!offer)
        throw new Error('Offer không tồn tại');
    if (offer.status !== 'OPEN')
        throw new Error('Offer đã được khớp hoặc hủy');
    if (offer.sellerId === buyerId)
        throw new Error('Không thể tự mua offer của mình');
    if (buyer.assets.TS < offer.priceTS)
        throw new Error('Không đủ TS để mua');
    // Transfer
